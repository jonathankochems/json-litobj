...
main :: IO ()
main = do text_json_nonstrict

text_json_nonstrict :: IO ()
text_json_nonstrict = hspec $ do
...
json_object :: Gen (JSObject JSValue)
json_object = do
   depth_bound <- choose(0,4)
   json_object_with_depth depth_bound

json_value_with_depth :: Int -> Gen JSValue
json_value_with_depth d = oneof [ fmap JSString $ json_string,
